...
def generate_user_report(modeladmin,request,queryset):
    ...
    response['Content-Disposition'] = 'attachement;filename=User Report '+str(date.today())+'.csv'
...
def generate_donor_report(modeladmin,request,queryset):
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachement;filename=Donor Report '+str(date.today())+'.csv'
    writer = csv.writer(response)
    writer.writerow(['Donor Name', 'Blood Group', 'Gender', 'Address','Mobile No', 'Date Created'])

    donors = queryset
    for donor in donors:
        writer.writerow([donor.user.user_name, donor.blood_group, donor.gender, donor.city, donor.mobile_no, donor.date_created])
